...
# scrapy crawl province_laws_86


class ProvinceLaw86Spider(scrapy.Spider):
    name = 'province_laws_86'
    ...
    def start_requests(self):
        ...
        start_url = "http://wjbb.sft.henan.gov.cn/rules/viewQueryAll.do?offset={}&cdid=402881fa2d1738ac012d173a60930017"
    ...
    def parse_article(self, response):
        ...
        item["legalCategory"] = "河南省政府-规章"
        item["legalPolicyName"] = tools.clean(response.meta['title'])
        item["legalPublishedTime"] = tools.clean(response.meta['time'])[1:-1]
        time.sleep(2)
        wenhao = re.search(r'[\(（](.+?)[）\)]', response.meta['title'])
        if wenhao:
            item["legalDocumentNumber"] = wenhao.group(1)
