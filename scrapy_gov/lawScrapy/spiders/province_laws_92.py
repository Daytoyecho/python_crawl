...
# scrapy crawl province_laws_92


class ProvinceLaw92Spider(scrapy.Spider):
    name = 'province_laws_92'
...
#-------------------------------------------------------------#
    ...
    def start_requests(self):
        ...
        start_url = ['http://jrjg.ln.gov.cn/zfxxgk_146759/fdzdgknr/lzyj/bbmgfxwj/']
    ...
    def parse_article(self, response):
        ...
        item["legalCategory"] = "辽宁省地方金融监督管理局-规范性文件"

        item["legalPolicyName"] = tools.clean(response.meta['title'])
        item["legalPublishedTime"] = tools.clean(response.meta['time'])

        # 注释：文号处理太麻烦，总共就7个，你在数据库里复制粘贴一下算了
        ...
        if tools.isWeb(response.url):
            ...
            fujian = response.xpath('//div[@class="news-attachment"]/ul//li/a/@href').extract()
            fujian_name = response.xpath('//div[@class="news-attachment"]/ul//li/a[@href]').xpath('string(.)').extract()
