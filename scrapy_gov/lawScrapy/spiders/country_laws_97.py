...
class CountryLaw97Spider(scrapy.Spider):
    name = 'country_laws_97'
    allowed_domains = ['csrc.gov.cn']
    url_list = []
    count = 0

    def start_requests(self):
        start_url = 'http://www.csrc.gov.cn/searchList/cd11df89f5894c1eac37ae37cc11e369?_isAgg=true&_isJson=true&_pageSize=10&_template=index&_rangeTimeGte=&_channelName=&page={}'
    ...
    def parse_dictionary(self, response):
        ...
        for i in data_list:
            ...
            law_document_number = ""
            for item in i['domainMetaList'][0]['resultList']:
                if item['name'] == "文号":
                    law_document_number = item["value"]
                    break
            law_url = tools.getpath(law_url, response.url)
            self.count += 1
            print(self.count)
            if law_url not in self.url_list:
                yield scrapy.Request(law_url, self.parse_article, meta={"title": law_title, "time": law_time, "documentNumber": law_document_number}, dont_filter=False)
    ...
    def parse_article(self, response):
        ...
        if tools.isWeb(response.url):
            ...
            content = re.sub(r'<div class="xxgk-down-box">[\s\S]+?<div class="clear"></div>[\s\S]+?</div>', r"", content, flags=re.I)
