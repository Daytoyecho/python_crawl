...
class CountryLaw106Spider(scrapy.Spider):
    name = 'country_laws_106'
    ...
    def start_requests(self):
        ...
        with open("lawScrapy/spiders/c106.json", 'r', encoding="utf-8") as f:
            data = json.load(f)
        ...
        for item in data:
            ...
            if url not in self.url_list:
                if url[:5] == "https":
                    proxy = a_bu_yun['https']
                else:
                    proxy = a_bu_yun['http']
                yield scrapy.Request(url, self.parse_article, meta={"title": law_title, "time": law_time, "document_number": law_document_number, 'proxy': proxy}, dont_filter=False, headers=tools.header)

    def parse_article(self, response):
        item = LawscrapyItem()
        item["legalUrl"] = response.url
        item["legalProvince"] = "深圳证券交易所"
        item["legalCategory"] = "深交所-本所业务规则全部规则"
