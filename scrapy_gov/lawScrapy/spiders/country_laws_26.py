...
# scrapy crawl country_laws_26


class CountryLaw26Spider(scrapy.Spider):
    name = 'country_laws_26'
    ...
    def start_requests(self):
        base = 'https://fuwu.most.gov.cn/html/tztg/index_{}.html'
        start_url = ['https://fuwu.most.gov.cn/html/tztg/']
    ...
    def parse_dictionary(self, response):
        law_url_list = response.xpath('//div[@class="right-min"]/ul/li/a/@href').extract()
        law_title = response.xpath('//div[@class="right-min"]/ul/li/a/@title').extract()
        law_time = response.xpath('//div[@class="right-min"]/ul/li/a/span[2]/text()').extract()

        for i in range(len(law_url_list)):
            tmpurl = tools.getpath(law_url_list[i], response.url)
            self.count += 1
            print(self.count)
            if tmpurl not in self.url_list and '.jhtml' not in tmpurl:
    ...
    def parse_article(self, response):
        ...
        item["legalCategory"] = "科学技术部-政务服务通知通告"
