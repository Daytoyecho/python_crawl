...
# scrapy crawl province_laws_110


class ProvinceLaw110Spider(scrapy.Spider):
    name = 'province_laws_110'
    ...
    def start_requests(self):
        ...
        base = "http://www.xa.gov.cn/gk/zcfg/szgg/list/{}.html"
        start_url = ['http://www.xa.gov.cn/gk/zcfg/szgg/1.html']
    ...
    def parse_dictionary(self, response):
        ...
        law_number = []
        for i in driver.find_elements_by_xpath('//table[@class="table table-striped"]/tbody//tr/td[1]/a'):
        ...
        for i in range(len(law_url_list)):
            ...
            if tmpurl not in self.url_list:
                yield scrapy.Request(tmpurl, self.parse_article, meta={"title": law_title[i], "number": law_number[i]}, dont_filter=False, headers=tools.header)

    def parse_article(self, response):
        item = LawscrapyItem()
        item["legalUrl"] = response.url
        item["legalProvince"] = "西安市"
        item["legalCategory"] = "西安市政府-市政公告"
        ...
        if tools.isWeb(response.url):
            ...
            # 处理时间
            shijian = response.xpath('//div[@class="file-table hidden-xs hidden-sm"]//tr[3]/td[4]/text()').extract_first()
            if shijian:
                item["legalPublishedTime"] = time.strftime("%Y-%m-%d", time.strptime(shijian, "%Y-%m-%d %H:%M"))
            else:
