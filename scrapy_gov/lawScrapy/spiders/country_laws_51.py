...
# scrapy crawl country_laws_51


class CountryLaw51Spider(scrapy.Spider):
    name = 'country_laws_51'
    ...
    def start_requests(self):
        ...
        for data in formdata:
            yield scrapy.Request(start_url, body=json.dumps(data), method='POST', callback=self.parse_dictionary, dont_filter=False, headers=header)
    ...
    def parse_dictionary(self, response):
        ...
        for item in data_list:
            ...
            if base_article_url.format(tmpform['pkid']) not in self.url_list:
                self.count += 1
                print(self.count)
                yield scrapy.Request(BASEURL, body=json.dumps(tmpform), method='POST', callback=self.parse_article, meta={"title": law_title, 'time': law_time, 'documentNumber': law_document_number, 'pkid': tmpform['pkid']}, dont_filter=False, headers=header)
    ...
    def parse_article(self, response):
        ...
        item["legalCategory"] = "司法部-行政规范性文件"
