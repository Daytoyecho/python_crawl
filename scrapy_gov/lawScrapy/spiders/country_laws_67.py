...
# scrapy crawl country_laws_67


class CountryLaw67Spider(scrapy.Spider):
    name = 'country_laws_67'
    ...
    def start_requests(self):
        BASEURL = 'http://www.mofcom.gov.cn/article/b/g/?{}'
        start_url = ['http://www.mofcom.gov.cn/article/b/g/']
    ...
    def parse_dictionary(self, response):
        ...
        for item in data_list:
            ...
            if tmpurl not in self.url_list:
                time.sleep(0.3)
                yield scrapy.Request(tmpurl, self.redirect, meta={"title": law_title}, dont_filter=False, headers=tools.header)

    def redirect(self, response):
        if not re.search(r'}var _cofing1={href:"([^"]+)"', response.xpath('//body').extract_first()):
            time.sleep(0.3)
            yield scrapy.Request(response.url, self.parse_article, meta={"title": response.meta['title']}, dont_filter=False, headers=tools.header)
        else:
            tmpurl = re.search(r'}var _cofing1={href:"([^"]+)"', response.xpath('//body').extract_first()).group(1)
            if tmpurl not in self.url_list:
                time.sleep(0.3)
                yield scrapy.Request(tmpurl, self.parse_article, meta={"title": response.meta['title']}, dont_filter=False, headers=tools.header)
    ...
    def parse_article(self, response):
        ...
        item["legalCategory"] = "商务部-中国对外经济贸易文告"
        ...
        if tools.isWeb(response.url):
            tmpt = re.search(r'var tm = "([^"]+?)"', response.xpath('//body').extract_first()).group(1)
            item["legalPublishedTime"] = time.strftime("%Y-%m-%d", time.strptime(tmpt.strip(), "%Y-%m-%d %H:%M:%S"))
