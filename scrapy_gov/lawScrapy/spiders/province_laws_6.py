...
# scrapy crawl province_laws_6


class ProvinceLaw6Spider(scrapy.Spider):
    name = 'province_laws_6'
    ...
    def start_requests(self):
        ...
        start_url = ['http://kw.beijing.gov.cn/col/col2384/index.html']
    ...
    def parse_article(self, response):
        ...
        item["legalCategory"] = "北京市科学技术委员会-北京市科技法规规章文件"
        ...
        if tools.isWeb(response.url):
            ...
            try:
                tmpnumber = response.xpath('//li[@class="fwzh"]/span').xpath('string(.)').extract_first()
            except:
                pass
            if re.search("〔", content):
                item["legalDocumentNumber"] = "人工审查"
            if re.search("[0-9]{4}", tmpnumber):
                item["legalDocumentNumber"] = tools.clean(tmpnumber)
