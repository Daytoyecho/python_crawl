...
# scrapy crawl country_laws_43


class CountryLaw43Spider(scrapy.Spider):
    name = 'country_laws_43'
    ...
    def start_requests(self):
        start_url = ['http://www.most.gov.cn/kjzc/gjsdkjzc/']
    ...
    def parse_dictionary(self, response):
        ...
        law_url_list = response.xpath('//div[@class="list"]//a[@href]/@href').extract()

        for i in range(len(law_url_list)):
            tmpurl = tools.getpath(law_url_list[i], response.url)
            yield scrapy.Request(tmpurl, self.tmp, dont_filter=False, headers=tools.header)

    def tmp(self, response):
        law_url_list = response.xpath('//div[@class="list"]//a/@href').extract()
        law_title = response.xpath('//div[@class="list"]//a/text()').extract()
    ...
    def parse_article(self, response):
        ...
        item["legalCategory"] = "科学技术部-国家试点科技政策"
