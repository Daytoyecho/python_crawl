...
# scrapy crawl country_laws_19


class CountryLaw19Spider(scrapy.Spider):
    name = 'country_laws_19'
    ...
    def start_requests(self):
        base = 'http://www.most.gov.cn/satp/kjzc/zh/index_{}.html'
        start_url = ['http://www.most.gov.cn/satp/kjzc/zh/']
    ...
    def parse_article(self, response):
        ...
        item["legalCategory"] = "科学技术部-科技政策综合"
        ...
        if tools.isWeb(response.url):
            ...
            # if not content:
            #     content = response.xpath('//ul[@class="dp_lia"]').extract_first()
            #     fujian = response.xpath('//ul[@class="dp_lia"]//a/@href').extract()
            #     fujian_name = response.xpath('//ul[@class="dp_lia"]//a[@href]').xpath('string(.)').extract()
            # if not content:
