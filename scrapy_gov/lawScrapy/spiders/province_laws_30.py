...
# scrapy crawl province_laws_30


class ProvinceLaw30Spider(scrapy.Spider):
    name = 'province_laws_30'
    allowed_domains = ['gz.gov.cn']
    url_list = []
    count = 0

    def start_requests(self):
        start_url = ["http://jrjgj.gz.gov.cn/zwgk/xxgk/xxgkml/gfxwj/index.html",
                     "http://jrjgj.gz.gov.cn/zwgk/xxgk/xxgkml/gfxwj/index_2.html"]
    ...
    def parse_article(self, response):
        ...
        item["legalCategory"] = "广州市地方金融监督管理局-规范性文件"
        ...
        if tools.isWeb(response.url):
            ...
            for i in response.xpath('//div[@class="info_cont"]//p').xpath('string(.)').extract():
                if not i:
                    continue
                if len(i) < 20:
                    tmpn = re.search(r"([^\(\)\（\）]{0,8}[\[〔][0-9]{4}[〕\]][0-9]+号)", tools.clean(i))
                    if tmpn:
                        item["legalDocumentNumber"] = tmpn.group(1)
                    else:
                        tmpn = re.search(r"(第[0-9]+号)", tools.clean(i))
                        if tmpn:
                            item["legalDocumentNumber"] = tmpn.group(1)
