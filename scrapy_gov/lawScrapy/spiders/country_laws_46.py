...
# scrapy crawl country_laws_46


class CountryLaw46Spider(scrapy.Spider):
    name = 'country_laws_46'
    ...
    def parse_dictionary(self, response):
        ...
        for i in data_list:
            ...
            if tmpurl not in self.url_list:
                yield scrapy.Request(tmpurl, self.parse_article, meta={"title": law_title, 'time': law_time, 'documentNumber': law_document_number}, dont_filter=False, headers=tools.header)

    def parse_article(self, response):
        item = LawscrapyItem()
        item["legalUrl"] = response.url
        item["legalProvince"] = "中华人民共和国工业和信息化部"
        item["legalCategory"] = "中华人民共和国工业和信息化部-公告"
        ...
        if tools.isWeb(response.url):
            try:
                ti = response.xpath('//*[@class="xxgk-span4"]/text()').extract_first()
            ...
            fujian.extend(response.xpath('//*[@id="con_con"]//iframe/@fileurl').extract())
            fujian_name.extend(['', ''])
